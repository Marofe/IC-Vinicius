function [rmse, hx, trP, euler] = run_EnKF_Lie(N, time, gps_time, hx, trP, P, Pqq, Prr, u, alpha, beta, kappa, L, Cen, y, leverarm, M, euler, ref) %#codegen
% RUN_ENKF_LIE
% Executes the Ensemble Kalman Filter on Lie Groups (EnKF-Lie).
%
% NOTE: This filter calculates sigma points only once at initialization (k=1)
% to construct a permanent ensemble of state trajectories, then propagates
% each ensemble member over time and updates individual members via the Kalman gain.
% On non-linear Lie group manifolds without regular resampling/inflation,
% a small fixed ensemble tends to suffer from ensemble collapse / particle deprivation,
% leading to elevated estimation error (e.g. RMSE ~ 77).
...
for k = 1:N-1
    ...
    if (abs(time(k+1) - gps_time(nk)) < dt)
        [hx_upd, P_upd, G_ensemble] = update_EnKF_Lie(hx_pred, P_pred, G_ensemble, Wm, Wc, Prr, y(:,nk), leverarm, L);
        hx(:,:,k+1) = hx_upd;
        P(:,:,k+1)  = P_upd;
        if nk < M
            nk = nk + 1;
    ...
    if ~mod(k, log_interval)
        fprintf('running the EnKF-Lie... %.1f%%\n', 100*k/N);
    end
end
