...
% Reference: Giorgio M. Magalhães et al. (CBA 2018)
...
for k = 1:N-1
    ...
    if (nk <= M) && (abs(time(k+1) - gps_time(nk)) < dt / 2)
        [hx(:, :, k+1), P(:, :, k+1)] = update_UKF_Lie(hx(:, :, k+1), P(:, :, k+1), Pqq, Prr, y(:, nk), G_pred, Chi_R, alpha, beta, kappa, leverarm, L);
        nk = nk + 1;
    end
