...
% Re-compiles run_UKF_Lie_mex if missing or if run_hybrid_EFK_UKF_Lie.m was modified
sourceFile = which('run_hybrid_EFK_UKF_Lie.m');
mexFile = which(['run_hybrid_EKF_UKF_Lie', '.', mexext]);
...
if isempty(mexFile)
    ...
    if sourceInfo.datenum > mexInfo.datenum
        disp('run_hybrid_EFK_UKF_Lie.m modified since last build. Rebuilding MEX...');
        needsRebuild = true;
    end
end
...
[rmse_opt, hx, trP, euler] = run_hybrid_EKF_UKF_Lie_mex(N, time, gps_time, hx, trP, P, Pqq, Prr, u, alpha_opt, beta, kappa, L, Cen, y, leverarm, M, euler, ref);
