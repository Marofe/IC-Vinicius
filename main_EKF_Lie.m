...
%% Re-compiles run_EKF_Lie_mex if missing or if run_EKF_Lie.m was modified
sourceFile = which('run_EKF_Lie.m');
mexFile = which(['run_EKF_Lie_mex', '.', mexext]);
needsRebuild = false;
...
if isempty(mexFile)
    ...
    if sourceInfo.datenum > mexInfo.datenum
        disp('run_EKF_Lie.m modified since last build. Rebuilding MEX...');
        needsRebuild = true;
    end
end
...
tic;
% Use native 'run_EKF_Lie' for line-by-line math breakdown
% Use mex version 'run_EKF_Lie_mex' for total C execution time
[hx, trP, euler] = run_EKF_Lie_mex(N, time, gps_time, hx, trP, P, Pqq, Prr, u, Cen, y, leverarm, M, euler);
profile off;                 % Stop tracking before plotting starts
profile viewer;              % Automatically open the profiler report window
EKFTime = toc;

fprintf('EKF-Lie MEX Execution Time: %.4f seconds\n', EKFTime);
...
save(['Workspaces/sol_EKF_lie_' trajectory '.mat'], 'hx_EKF_lie');
save(['Workspaces/full_EKF_lie_' trajectory '_workspace.mat']);
