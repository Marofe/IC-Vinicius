...
\node[sensor, right=3.5cm of imu] (gps) {GNSS Receiver\\$(\mathbf{y}_k)$};
\node[box, below=1.2cm of imu] (omega) {\texttt{Omega.m}\\Kinematics $\Omega$};
\node[box, right=0.9cm of omega] (sigpred) {\texttt{SigmaPointsLie}\\$\Xi$ generation};
\node[box, right=0.9cm of sigpred] (pred) {\texttt{prediction}\\UKF-Lie};
\node[box, right=0.9cm of pred] (upd) {\texttt{Update}\\UKF-Lie};
\node[box, below=1.3cm of sigpred] (mean) {\texttt{media\_nula\_g}\\Mean on $\mathcal{G}$};
\node[box, right=0.9cm of mean] (cov) {\texttt{covariancias}\\$\mathbf{P}_{gh},\mathbf{P}_{hh}$};
\node[output, below=1.3cm of pred] (state) {Nav. State $\bX_{k|k}$};
\node[output, right=0.9cm of state] (euler) {Euler Angles\\$(\phi,\theta,\psi)$};
\draw[arr] (imu) -- (omega);
\draw[arr] (omega) -- (sigpred);
\draw[arr] (sigpred) -- (pred);
\draw[arr] (pred) -- (upd);
\draw[arr] (gps) -- ++(0,-1.8) -| (upd);
\draw[arr] (pred) -- (mean);
\draw[arr] (mean) -- (cov);
\draw[arr] (cov) -- (upd);
\draw[arr] (upd) -- (state);
\draw[arr] (state) -- (euler);
\draw[arr] (upd.south) -- ++(0,-0.4) -- ++(-5.0,0) |- (sigpred.west);
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  start/.style={draw=lieblue, fill=lieblue, text=white, rounded corners=15pt,
                minimum width=6.5cm, minimum height=0.7cm, font=\small\bfseries},
  step/.style={draw=lieblue!70!black, fill=liegray, rounded corners=4pt,
...
\node[step, below=0.45cm of s0] (s1) {Build augmented covariance $\mathbf{P}_\eta=\mathrm{blkdiag}(\bP,\bQ,\bR)\in\mathbb{R}^{33\times33}$};
\node[step, below=0.42cm of s1] (s2) {Cholesky: $\mathbf{S}=\mathrm{chol}(\mathbf{P}_\eta)^\top$ (regularise if non-PD)};
\node[step, below=0.42cm of s2] (s3) {Generate $2L+1=67$ sigma points $\bm\xi^{(i)}$};
\node[step, below=0.42cm of s3] (s4) {Split: $\bm\xi^{(i)}\to(\bm{e}^{(i)},\mathbf{q}^{(i)},\mathbf{r}^{(i)})$};
\node[step, below=0.42cm of s4] (s5) {Pre-compute $(\lambda_0,g_0,\mathbf{C}_{en},\mathbf{g}^e)$ from $\bX_{k-1}$ once};
\node[step, below=0.42cm of s5] (s6) {$\forall i$: lift $\mathcal{X}^{(i)}=\bX\Exp(\bm{e}^{(i)})$, propagate $\mathcal{X}_{k|k-1}^{(i)}$};
\node[step, below=0.42cm of s6] (s7) {Iterative Frechet mean $\to\bar{\mathbf{X}}_k$ (LU reuse)};
\node[step, below=0.42cm of s7] (s8) {$\bm\varepsilon^{(i)}=\Log(\bar{\mathbf{X}}_k^{-1}\mathcal{X}^{(i)})$, compute $\bP_{k|k-1}$};
\node[start, below=0.45cm of s8] (s9) {Output $(\bar{\mathbf{X}}_k,\bP_{k|k-1},\{\mathcal{X}^{(i)}\},\{\mathbf{r}^{(i)}\})$};
...
\begin{tikzpicture}[
  ...
  start/.style={draw=liegreen!80!black, fill=liegreen!70!black, text=white,
                ...
                minimum width=6.5cm, minimum height=0.7cm, font=\small\bfseries},
  step/.style={draw=liegreen!60!black, fill=liegreen!7, rounded corners=4pt,
               minimum width=6.5cm, minimum height=0.85cm,
               align=center, font=\small, line width=0.8pt},
  decide/.style={draw=lieaccent, fill=lieaccent!10, diamond, aspect=2.5,
...
\node[decide] (check) {GNSS epoch? $|t_k-t_{\mathrm{GPS}}|<\Delta t$};
\node[start, below=0.6cm of check] (u0) {Update Start};
\node[step, below=0.42cm of u0] (u1) {Meas. sigma pts $\mathcal{Y}^{(i)}\leftarrow h(\mathcal{X}^{(i)})+\mathbf{r}^{(i)}$};
\node[step, below=0.42cm of u1] (u2) {Predicted meas. mean $\bar{\mathbf{y}}$ (\texttt{media\_nula\_h})};
\node[step, below=0.42cm of u2] (u3) {Compute $\mathbf{P}_{hh}$, $\mathbf{P}_{gh}$ (\texttt{covariancias})};
\node[step, below=0.42cm of u3] (u4) {Kalman gain $\bK=(\mathbf{P}_{hh}^{-1}\mathbf{P}_{gh}^\top)^\top$};
\node[step, below=0.42cm of u4] (u5) {Innovation $\bm\delta_t=\mathbf{y}_k-\bar{\mathbf{y}}$, $\bm\varepsilon=\bK\bm\delta_t$};
\node[step, below=0.42cm of u5] (u6) {State retraction: $\bX_{k|k}=\bar{\mathbf{X}}_k\Exp(\bm\varepsilon)$};
\node[step, below=0.42cm of u6] (u7) {Cov. update: $\bP_{k|k}=\bP_{k|k-1}-\bK\mathbf{P}_{gh}^\top$, symmetrize};
\node[start, below=0.45cm of u7] (u8) {Output $(\bX_{k|k},\bP_{k|k})$};
\draw[sarr] (check) -- node[right,font=\small]{yes} (u0);
\draw[sarr] (check) -- ++(4,0) node[right,font=\small,lieaccent]{skip};
